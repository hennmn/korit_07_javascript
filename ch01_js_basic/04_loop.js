...
// // 이상과 같은 방식을 굳이 할 필요 없으니깐 반복문 써야겠죠
// //  Java와 동일합니다.
...
// , 집어 넣어도 오류 안 납니다. 이렇게 집어 넣는 게 관행입니다.

/*
  이상의 코드 라인에서 주목해야 할 부분은 첫 번째, 마지막 index에 해당하는 element
  다음에도 ',' 가 찍혀있는데도 불구하고 오류가 발생하지 않는다는 점

  두 번째, Array의 element인 object의 마지막 property의 value값 다음에도 ','가 있지만
  오류가 발생하지 않는다는 점

  입니다.

  이거 추후에 property를 추가하거나 혹은 element를 추가하거나 할 때 전에 말한 것처럼 쉼표 치고 엔터 치고 다시 {} 혹은 property 추가를 하기 귀찮으니까
  맨마지막 element/property 여부와 관계없이 쉼표 찍어주는 게 매너가 된 게 아예 관행처럼 굳어진 겁니다. 그래서 오류로 처리하지도 않습니다.
*/
...
console.log(productList[0].name+ ' 가격과 ' + productList[2].name + '의 가격은 각각 '+ productList[0].price + '원과 ' + productList[2].price + ' 원으로, 그 합은 ' + (productList[0].price + productList[2].price) +' 원 입니다.');
...
/*
  ...
  [object Obeject],[object Obeject],[object Obeject]
  형식으로만 나오게 되고
...
//   for(let i = 0; j < outputList[i]; j++) {
//     console.log(outputList[i][j]);
//   }
// }
// 이 코드는 논리적으로 말이 되지 않습니다.
/*
  outputList의 element의 자료형은 객체이고, 객체에는 index number가 존재하지 않으므로 이중 for문을 통해서 객체 내부의 각 property를 확인하는 것이 불가능 합니다.
...
// 그래서 해당 key에 할당된 value를 추출하고 싶다면  117번 라인과 같은 방식으로 코드를 작성해야 합니다.
...
//   즉 person.key는 person.fName이 아니라 person.'fName'이라는 점이 문제입니다.  필드명이 아니라 리터럴 데이터라서 바로 fName을 쓰는 게 안됨
 
// }
 // 그러면 console.log(person.key); 는 불가능할까요?

// 그래서 굳이 #1,2로 두 가지 방식을 통해서 객체의 value를 조회할 수 있습니다.
...
// 객체의 property의 key를 추출할 수 있으니까, 마치 향상된 for 문처럼 여겨져서 
