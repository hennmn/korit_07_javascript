...
/* 
  8번 라인에 대한 해석을 위해서는 어제 배운 함수 표현식 관련한 부분을 알아겠네요.

  JS에서는 함수를 변수에 저장할 수 있었습니다. 
  그래서 객체명.함수명()으로 호출을 하게 될 거니까 저희가 배운 방식으로는 method라고 할 수 있겠습니다.
*/
...
/* 
  이상의 부분에서 주의할 점은 한계갑 ㅅ바로 앞까지만 불러온다는 점입니다. 그래서 31번  라인의 경우
  12번 index까지라고 했을 때 Banana라는 결과값이라고 착각할 수 있습니다.

  즉, slice()의 두 번째 매개변수를 for문에서의 한계값 설정할 때 제가 이상/이하 보다는 초과/ 미만을 사용하는 경우가 많은데, 그 이유가 여기서도 적용됩니다...
*/
let result4 = str2.slice(7);  // 매개변수 하나일 땐 시작 인덱스임
 ...
 */
let result5 = str2.slice(-12);   // 매개변수 하나일 땐 시작 인덱스임
console.log(result5);

/*
  그러면 굳이 마이너스 인덱스를 써야하는 경우를 가정하겠습니다.

  자동차 차량 번호판 예시를 들었을 때
  12일 5678
  370수5690
...
console.log(result6);
// 마이너스 인덱스를 지원하지 않기 때문에 결과값이 예상과 닥릅니다.
let result7 = str.substring(-12);
...
/*
  87번 라인의 경우 처음 보는 형태입니다.
  JS 정규식 표현식을 썼는데 추후 설명 예정이고 i는 insensitive의 축약어로 덜 깐깐하게 굴겠다는 겁니다.
  대소문자 구분 안 하고 그냥 찾는다는 의미.
*/

let rpdStc3 = eStc.replace(/seoul/g, 'Busan');
console.log(rpdStc3);
// 결과값 : Please visit Busan and Busan
// /g는 global 약자로, 맨 앞에 것만 바꾸는 게 아니라 전부 다 바꾼다는 의미입니다.
...
/*
  실무 사용 예시
  우리나라에서는 이름이 성이름이 붙어있지만 다른 나라에서는 이름 성 형태나 / 성 이름에서 공백으로 구분하는 경우가 있습니다. 이상의 경우에
